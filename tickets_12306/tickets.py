...
train_date={3}'.format(raw_train['train_no'],raw_train['from_station_no'],\
raw_train['to_station_no'],date_time)
                ...
                price_url ='https://kyfw.12306.cn/otn/leftTicket/queryTicketPrice?\
train_no={0}&from_station_no={1}&to_station_no={2}&seat_types=1413&\
train_date={3}'.format(raw_train['train_no'],raw_train['from_station_no'],\
raw_train['to_station_no'],date_time)
                price_json = requests.get(price_url,verify=False) 
                available_price = price_json.json()['data']
...
raw_train['to_station_no'],date_time)
            ...
            if not self.options or initial in self.options:
                ...
                train = [
                    ...
                    '\n'.join([Fore.WHITE+raw_train['zy_num']+Fore.RESET,Fore.RED+'%s'%(arg[1])+Fore.RESET]),
                    '\n'.join([Fore.WHITE+raw_train['ze_num']+Fore.RESET,Fore.RED+'%s'%(arg[2])+Fore.RESET]),
                    '\n'.join([Fore.WHITE+raw_train['rw_num']+Fore.RESET,Fore.RED+'%s'%(arg[3])+Fore.RESET]),
                    '\n'.join([Fore.WHITE+raw_train['yw_num']+Fore.RESET,Fore.RED+'%s'%(arg[4])+Fore.RESET]),
                    '\n'.join([Fore.WHITE+raw_train['yz_num']+Fore.RESET,Fore.RED+'%s'%(arg[5])+Fore.RESET]),
                    '\n'.join([Fore.WHITE+raw_train['wz_num']+Fore.RESET,Fore.RED+'%s'%(arg[6])+Fore.RESET]),
                ]
            yield train
